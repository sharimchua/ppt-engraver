...
- **Weave**: A structural container representing a voice, part, or section composed of hierarchical stitches (`stitch: [...]`) organized via `layout: concatenate` (sequential), `layout: parallel` (simultaneous layering / polyphony), or `layout: parallelPeriod` (period-matched polyrhythms).
