...
- **Score Header Pulse Signature (`showPulseSignature: true` / `pulseSignature`)**: Rendered on a **separate line** below the key anchor. Each syllable in the pulse string is parsed and emitted as a full-color PPT Solfège glyph stencil (`make-solfege-glyph`) in its chromatic color, preceded by a small "Pulse:" label. Implemented as `\markup \column { keyAnchorBody pulseRow }` where each row body is a `\line \vcenter { ... }` expression (no nested `\markup` prefix).
- **Piano Triangle Chord Spellings, Key Anchor & Key Signature (`show: [chordTriangles]` / `showChordTriangles: true`, `keyAnchorStyle: 'pianoTriangle' | 'both'`, `show: [keySignature]` / `showKeySignature: true`)**: When `show: [chordTriangles]` is configured, `\new ChordNames` emits vector Piano Triangle chord hand shapes enclosed in a rectangular outline colored in the chord's Solfège root color (e.g. Red for $E\flat\text{m}$, Pink for $D\flat$, Purple for $B$, Blue for $B\flat$) for clear visual grouping across rapid changes. Standard `show: [chordNames]` emits traditional lead sheet symbols (e.g. `Ebm`, `Db`, `B`, `Bb`). When `keyAnchorStyle: 'pianoTriangle'`, the score header key anchor replaces the letter name with the single triangle symbol for the tonic (e.g. `[Do Glyph] = [D2 Triangle]`). When `showKeySignature: true` / `show: [keySignature]`, the score header emits the tetrachord-chained sequence of triangles centered around the tonic `[5, 6, 7] + [1] + [2, 3, 4]`.
