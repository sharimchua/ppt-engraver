...
- **Mid-Score Weave Boundary Key/Scale Changes**: When traversing consecutive weave boundaries in `melodyVoice` and `harmonyVoice`, if the scale changes (e.g. from `Do` / Major to `La` / Minor) and `showKeySignature` is active, LilyPond emits in-place `\key <tonic> \<newMode>` at the section barline.
